		    ...
		    label="地址">
		     <template slot-scope="scope">
                ...
                <el-button v-if="isAuth('dingdanxinxi','配送') && contents.tableBtnIcon == 0" type="success" size="mini" @click="dingdanpeisongCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'配送':'' }}</el-button>
                                                <el-button v-if="isAuth('dingdanxinxi','订单取消') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="dingdanquxiaoCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'订单取消':'' }}</el-button>
                <el-button v-if="isAuth('dingdanxinxi','订单取消') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="dingdanquxiaoCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'订单取消':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('dingdanxinxi','订单取消') && contents.tableBtnIcon == 0" type="success" size="mini" @click="dingdanquxiaoCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'订单取消':'' }}</el-button>
  ...
  methods: {
    ...
    contentPageStyleChange(){
      ...
      this.$nextTick(() => {
        ...
        dingdanquxiaoCrossAddOrUpdateHandler(row,type){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
